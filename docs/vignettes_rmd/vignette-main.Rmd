...
author: "Qianyu Dong, Zehang Richard Li, Yunhan Wu, Andrea Boskovic, Jon Wakefield"
date: "`r Sys.Date()`"
...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, tidy = FALSE, eval=TRUE, cache = FALSE, out.width = "100%")
```
...
Then in Section \@ref(sec:vis) we show several tools for visualization of different estimates, and in Section  \@ref(sec:aggregation) we discuss how to aggregate the result for a given model to produce estimates at higher admin levels.
...
+ **Population map (optional)**: These can be downloaded from WorldPop site^[[https://hub.worldpop.org/project/categories?id=8](https://hub.worldpop.org/project/categories?id=8)].These data are only required for the stratified cluster-level model described in Section \@ref(sec:cluster-level-strat) and for result aggregation described in Section \@ref(sec:aggregation).
...
data(indicatorList)
num <- nrow(indicatorList)
```

Currently, the `surveyPrev` package supports `r num` indicators. The full list of indicators and their IDs can be found in the `indicatorList` dataset. The `ID` column corresponds to the standardized indicator IDs used in the DHS API, while the `Description` and `Topic` columns give a short description and the DHS chapter each indicator belongs to. In previous versions of **surveyPrev**, a small set of indicators could also be referred to by shorter alternative IDs (for example, `ancvisit4+`); these alternative IDs are no longer used, and the standard DHS indicator IDs should be used instead. The full list of indicators is summarized in Table \@ref(tab:indic).
...
if(is_html_output()){
  kbl(indicatorList[, c("ID", "Description", "Topic")], align = 'lll',
...
Details on more common DHS indicators can be found in the Guide to DHS Statistics at [https://dhsprogram.com/data/Guide-to-DHS-Statistics/](https://dhsprogram.com/data/Guide-to-DHS-Statistics/). User-specified function can also be used to process raw DHS data into new indicators. Please refer to the vignette on _Creating Customized Indicators for **surveyPrev**_ for more details.
...
A Fay-Herriot model at Admin 2 level can be fitted by treating areas without direct estimates as missing data, though it is usually not recommended due to data sparsity. In addition, numerical issues arise when the design-based variance of a direct estimate is close to zero and the logit precision becomes too large. A potential fix for this issue is to identify regions with too small design variance(< 1e-30), and delete the clusters in these regions, before fitting the model. However, this step creates additional bias in the results if the number of clusters deleted is large.
...
## Unstratified model {#sec:cluster-level-unstrat}
...
## Stratified model {#sec:cluster-level-strat}
...
The `directEST()` function allows aggregation by survey weight directly by specifying `weight = "survey"`, without going through the steps discussed in Section \@ref(sec:survey-weight-fraction). We compute the direct estimates at admin1 level and aggregate them to national estimates. When using survey weight, the aggregated point estimate at national level is the same as the national level direct estimate, but the uncertainty calculations are different, as discussed above.
