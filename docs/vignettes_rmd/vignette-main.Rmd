...
Then in Section \@ref(sec:vis) we show several tools for visualization of different estimates, and in Section  \@ref(sec:aggregation) we discuss how to aggregate the result for a given model to produce estimates at higher admin levels. For a more detailed discussion of the workflow of SAE analysis using the **surveyPrev** package, see [@dong2026toward].
...
if(is_html_output()){
  kbl(indicatorList[1:20, c("ID", "Description", "Topic")], align = 'lll',
...
res_ad0_rural <- directEST(data=data,
                   ...
                   admin=0,
                   strata="rural")
list(all = res_ad0$res.natl, 
      urban = res_ad0_urban$res.natl, 
      rural = res_ad0_rural$res.natl)
```
...
Design-based variance at Admin 2 level can be unstable due to data sparsity, which sometimes creates numerical issues for Fay-Herriot model. A fix for this variance issue is described in [@wakefield2026variance] and is enabled by setting `var.fix = TRUE`.
