...
Let's take **Modern contraceptive prevalence rate (Married women currently using any modern method of contraception)** as an example. For users familiar with the standard indicators defined by the [DHS Data Indicator API](https://api.dhsprogram.com/rest/dhs/indicators?returnFields=IndicatorId,Label,Definition&f=html), the indicator ID is "FP_CUSM_W_MOD". We will go through the steps to create the customized function below.
...
For our example, we can either search "FP_CUSM_W_MOD", or "contraceptive", and it is in chapter 7: family planning.
...
The example below creates a `fp_cruse_mod` function for "Modern contraceptive prevalence rate (Married women currently using any modern method of contraception)", which can be recognized by the `getDHSindicator` function later.
