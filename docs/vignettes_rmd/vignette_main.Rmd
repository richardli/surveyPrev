...
library(devtools)
install_github("richardli/LINsurveyPrev")
```
...
The function also produce aggregated estimates to upper administrative levels when `aggregation = T`. Let i be the lower admin level and k be the upper admin level, then aggregated estimates for area k is 

$$ \hat p^{HT}_{k}=\frac {\sum_{i \in k} \hat p^{HT}_{i}E_i}{\sum_{i \in k} \text{E}_i},$$
where $E_i=\text{population}_{i}\times \textbf{1}(\hat p^{HT}_{i} \neq \text{NA})$. When there is no missing area i in k, $\hat p^{HT}_{k}$ is the weighted average using population fractions. When Admin i in k is missing, then the $\hat p^{HT}_{k}$ is the weighted average of the rest areas in k. 
...
## Visualizing Prevalence Estiamtes {#sec:vis}
...
```{r ,fig.dim = c(6, 4),fig.cap="Comparing Admin 1 direct estimates, Admin 1 Fay–Herriot, Admin 1 and Admin 2 unstratified cluster-level estimates"}
...
```{r, warning=FALSE,echo=FALSE,fig.show="hold", fig.height = 5, fig.width = 10,fig.cap="Comparing Admin 1 direct estimates and Admin 1 Fay–Herriot"}
s1<-scatterPlot(
...
```{r, warning=FALSE,echo=FALSE,fig.show="hold", fig.height = 5, fig.width = 10,fig.cap="Comparing Admin 1 direct estimates and Admin 1 unstratified cluster-level estimates"}
s1<-scatterPlot(
res1=res_ad1$res.admin1,
...
## Admin 2 Cluster-Level Estimates with Urban/Rural Stratification {#sec:cluster-level2}
...
```{r ,warning=FALSE,echo=FALSE,fig.show="hold",fig.dim = c(6, 4),fig.cap="Comparing Admin 2 direct estimates, Admin 2 Fay–Herriot, Admin 2 unstratified and stratified cluster-level estimates"}
...
```{r,fig.show='hold',fig.dim = c(6, 4), fig.cap="survey weight ratio and population ratio at Admin 1 and Admin 2"}
...
```{r, fig.show='hold',fig.dim = c(12, 6),tidy=FALSE, fig.cap="Admin 2 survey weight ratio and 15-49 female population ratio"}
...
```{r, tidy = FALSE, fig.cap="Comparing different aggregated direct estimates at national level"}
...
```{r,fig.cap="Comparing different aggregated Fay-Herriot estimates at national level"}
...
```{r,tidy = FALSE, fig.cap="Comparing different aggregated cluster-level estimates at national level"}

intervalPlot(admin = 0, group=FALSE, compare=TRUE, model = list(
            "Admin 0 FH model"=res_ad0,
             "Aggregated Admin 2 cluster model by survey"=cl_res_ad2.suevey,
             "Aggregated Admin 1 cluster model by survey"=cl_res_ad1.survey,
             "Aggregated Admin 2 cluster model by pop"=cl_res_ad2,
             "Aggregated Admin 1 cluster model by pop"=cl_res_ad1,
             "Aggregated Admin 2 stratified cluster model by pop"=cl_res_ad2_T,
             "Aggregated Admin 1 stratified cluster model by pop"=cl_res_ad1_T))
...
```{r,fig.show='hold',fig.dim = c(6, 4),tidy = FALSE, fig.cap="Comparing aggregated Admin 2 stratified cluster-level estiamtes, aggregated Admin 2 direct estiamtes, aggregated Admin 2 Fay-Herriot, and Admin 1direct estiamtes"}
...
```{r,fig.show='hold',fig.dim = c(6, 4), tidy = FALSE, fig.cap="Comparing different model estimates at national level"}
