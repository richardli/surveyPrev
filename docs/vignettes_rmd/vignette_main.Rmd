...
```{r ,fig.cap="Comparing Admin 1 direct estimates, Admin 1 Fay–Herriot, Admin 1 and Admin 2 unstratified cluster model estimates"}
...
g1 <- mapPlot(data = res_ad1$res.admin1, geo = poly.adm1, by.data = "admin1.name", by.geo = "NAME_1", variable = "direct.est", legend.label = "Admin 1 Direct", ylim =ylim, removetab = TRUE)

g2 <- mapPlot(data = smth_res_ad1_bym2$res.admin1, geo = poly.adm1, by.data = "admin1.name", by.geo = "NAME_1", variable = "mean", legend.label = "Admin 1 Fay–Herriot", ylim =ylim, removetab = TRUE)

g3 <- mapPlot(data = cl_res_ad1$res.admin1, geo = poly.adm1, by.data = "admin1.name", by.geo = "NAME_1", variable = "mean", legend.label = "Admin 1 unstratified", ylim = ylim, removetab = TRUE)

g4 <- mapPlot(data = cl_res_ad2$res.admin2, geo = poly.adm2, by.data = "admin2.name.full", by.geo = "admin2.name.full", variable = "mean", legend.label = "Admin 2 unstratified", ylim = ylim, removetab = TRUE)
...
For `directEST()`, if we set `admin = 2`, the output includes results at the Admin 2 level with `aggregation = FALSE` and aggregated results at both the Admin 1 and national level  with `aggregation = TRUE`. For countries with a large number of Admin 2 areas, some of the direct estimates can be undefined if there is no cluster in the area.
...
problem_clusters <- cluster.info$data$cluster[cluster.info$data$admin2.name.full %in% problem_admin2]
...
The `compare=F` in `intervalPlot()`is specifically designed for cluster models with urban/rural stratification. It presents a comparison of Admin 2 estimates with credible intervals within an Admin 1 region. 
...
## Comparing Admin 2 Prevalence Estimates  {#sec:vis2}

We can compare the estimates on a map using `mapPlot()` function in **SUMMER** as before.

```{r ,fig.cap="Comparing Admin 2 direct estimates, Admin 2 Fay–Herriot, Admin 1 and Admin 2 stratified cluster model estimates"}
...
g1 <- mapPlot(data = admin.info1.survey$data, geo = poly.adm1, by.data = "admin1.name", by.geo = "NAME_1", variable = "r.SurveyWeight", legend.label = "rario SurveyWeight",removetab = TRUE, ylim = ylim)
g2 <- mapPlot(data= admin.info1$data, geo = poly.adm1, by.data = "admin1.name", by.geo = "NAME_1", variable = "r.population", legend.label = "rario Population", removetab = TRUE, ylim = ylim)
...
g3 <- mapPlot(data = admin.info2$data, geo = poly.adm2, by.data = "admin2.name.full", by.geo = "admin2.name.full", variable = "r.population", legend.label = "rario population", removetab = TRUE, ylim = ylim2)

g4 <- mapPlot(data = admin.info2.survey$data, geo = poly.adm2, by.data = "admin2.name.full", by.geo = "admin2.name.full", variable = "r.SurveyWeight", legend.label = "rario SurveyWeight",  removetab = TRUE, ylim = ylim2)
...
Similarly, if we set `admin = 2`, the output includes results at the Admin 2 level with `aggregation = FALSE` and aggregated results at both the Admin1 and national level  with `aggregation = TRUE`, computed by weighting the Admin 2 level estimates by their population (as specified by `weight = "population"`). 
...
intervalPlot(admin = 0, group = FALSE, compare = TRUE, 
  ...
  "Aggregated Admin 2 direct estimate model by survey" =res_ad2agg_bysurvey,
  "Aggregated Admin 1 direct estimate model by survey" =res_ad1agg_bysurvey,
  "Aggregated Admin 2 direct estimate model by pop" = res_ad2agg,
  "Aggregated Admin 1 direct estimate model by pop" = res_ad1agg))
...
intervalPlot(admin = 0, group=FALSE, compare=TRUE, model = list(
            ...
            "Aggregated Admin 2 FH model by survey"=smth_res_ad2_agg.survey,
            "Aggregated Admin 1 FH model by survey"=smth_res_ad1_spatial.survey,
            "Aggregated Admin 2 FH model by pop"=smth_res_ad2_agg,
            "Aggregated Admin 1 FH model by pop"=smth_res_ad1_spatial
          ))
...
intervalPlot(admin = 0, group=FALSE, compare=TRUE, model = list(
            "Admin 0 FH model"=res_ad0,
             "Aggregated Admin 2 cluster model by survey"=cl_res_ad2.suevey,
             "Aggregated Admin 1 FH cluster by survey"=cl_res_ad1.survey,
             "Aggregated Admin 2 FH cluster by pop"=cl_res_ad2,
             "Aggregated Admin 1 FH cluster by pop"=cl_res_ad1,
             "Aggregated Admin 2 FH cluster stratified by pop"=cl_res_ad2_T,
             "Aggregated Admin 1 FH cluster stratified by pop"=cl_res_ad1_T
...
g4 <- mapPlot(data = smth_res_ad2_agg$agg.admin1, geo = poly.adm1, by.data = "admin1.name", by.geo = "NAME_1", variable = "mean", legend.label = "Admin 2 Smooth Direct", ylim = ylim, removetab = TRUE)
...
The `compare = FALSE` in `intervalPlot()`is specifically designed for cluster models with urban/rural stratification. It presents a comparison of Admin 2 estimates with credible intervals within an Admin1 region.
