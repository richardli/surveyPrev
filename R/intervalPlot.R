...
intervalPlot<-function(res){
  ...
  plot_fun <- function(dat) {
    ...
    ggplot(dat, aes(x = admin2.name, y = value,color=type)) +
        geom_point( aes(type = type), position = position_dodge(width = 0.5),size = .8) +
        geom_errorbar(aes(ymin = lower , ymax = upper), alpha = 0.5,position = position_dodge(width = 0.5)) +
        scale_color_manual(values = c("red", "blue", "darkgreen"))+
        geom_hline(  aes(yintercept =line1,linetype = "dashed"),color="grey30",size = .6) +
        geom_hline( aes(yintercept =line2, linetype = "solid"),color="grey30",size = .6) +
