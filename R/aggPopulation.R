...
#' @param by.adm the column name of column for Admin names for desired output Admin level, can be such as "NAME_1" or "NAME_2". Duplicated admin names are handled internally when by.adm.upper is specified
...
aggPopulation <- function(tiff, fact = 10, poly.adm, by.adm, by.adm.upper=NULL){
  ...
  if(!is.null(by.adm.upper)){
        adm <- over(loc_df, poly.adm)[, "admin2.name.full"]
  }else{
 ...
 if(!is.null(by.adm.upper)){
    ...
    colnames(tab)[colnames(tab) == "adm"] <- "admin2.name.full"
    tab = left_join(tab, poly.adm@data[,c( by.adm, by.adm.upper, "admin2.name.full")])
