##'WS_SRCE_H_USG Percentage of households whose main source of drinking water is an unprotected spring
##' ph_wtr_source in github "unprotected spring" = 42,
##' HR
...
WS_SRCE_H_USG<-function(Rdata){
  ...
  WASHdata <- WASHdata %>% mutate(WS_SRCE_H_USG = case_when(
    ph_wtr_source %in% c(42) ~ 1,
    ph_wtr_source %in% c(11, 12, 13, 14, 15, 21, 31, 41, 51, 61, 62, 65, 71, 72, 73,30, 32, 40, 43, 96) ~ 0,
    ph_wtr_source==99 ~ 99)) %>%
    set_value_labels(WS_SRCE_H_USG = c(
      "unprotected spring" = 1,
      "other" = 0,
      "missing" = 99)) %>%
    set_variable_labels(WS_SRCE_H_USG = "unprotected spring")





  colnames(HRdata)[colnames(HRdata) == 'WS_SRCE_H_USG'] <- 'value'
