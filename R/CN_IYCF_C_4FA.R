...
#     set_variable_labels(nt_bf_cont_2yr = "Continuing breastfeeding at 2 year (20-23 months) - last-born under 2 years")
  ...
  KRiycf <- KRiycf %>%
    ...
    mutate(CN_IYCF_C_4FA  = case_when(inrange(age,6,23) & foodsum<5 ~ 0 , inrange(age,6,23) & foodsum>=5~ 1)) %>%
    # mutate(CN_IYCF_C_4FA = case_when(
    #   data.table::inrange(age, 6, 23) & foodsum < 5 ~ 0,
    #   data.table::inrange(age, 6, 23) & foodsum >= 5 ~ 1
    # )) %>%
     #older surveys are 4 out of 7 food groups so the foodsum would add group2-group8 and the recode the sum for 4+ as yes
    set_value_labels(CN_IYCF_C_4FA = c("Yes" = 1, "No"=0  )) %>%
    set_variable_labels(CN_IYCF_C_4FA = "Child with minimum dietary diversity, 5 out of 8 food groups- last-born 6-23 months")

  colnames(KRiycf)[colnames(KRiycf) == 'CN_IYCF_C_4FA'] <- 'value'
