...
#'
#' @export
RH_DELP_C_PRV<- function(Rdata){

  #Manually added function by Qianyu
  #RH_DELP_C_PRV(five year version/ official id name)
  ...
  BRdata <- BRdata %>%
    mutate(RH_DELP_C_PRV =
             case_when(
               rh_del_pltype==2 ~ 1,
               rh_del_pltype %in% c(1,3,4,9)   ~ 0 )) %>%
    set_value_labels(RH_DELP_C_PRV = c("Yes" = 1, "No"=0)) %>%
    set_variable_labels(RH_DELP_C_PRV = "Live births by place of delivery: Health facility- private")

  colnames(BRdata)[colnames(BRdata) == 'RH_DELP_C_PRV'] <- 'value'
  return(BRdata)
}
