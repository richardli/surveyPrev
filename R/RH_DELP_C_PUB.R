...
#'
#' @export
RH_DELP_C_PUB<- function(Rdata){

  #Manually added function by Qianyu
   # RH_DELP_C_PUB(five year version/ official id name)
  ...
  BRdata <- BRdata %>%
    mutate(RH_DELP_C_PUB =
             case_when(
               rh_del_pltype==1 ~ 1,
               rh_del_pltype %in% c(2,3,4,9)   ~ 0 )) %>%
    set_value_labels(RH_DELP_C_PUB = c("Yes" = 1, "No"=0)) %>%
    set_variable_labels(RH_DELP_C_PUB = "Live births by place of delivery: Health facility- public")

  colnames(BRdata)[colnames(BRdata) == 'RH_DELP_C_PUB'] <- 'value'
  return(BRdata)
}
